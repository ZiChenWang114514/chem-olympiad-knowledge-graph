import type { Discipline, Exam, GraphData, SearchItem, Stats, Problem } from '../types'
...
export async function loadData() {
  const [taxonomy, exams, graph, search, statistics, ...yearFiles] = await Promise.all([
    get<{ disciplines: Discipline[]; relations: { id: string; name: string }[] }>('data/taxonomy.json'),
