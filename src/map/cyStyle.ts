...
export function buildCyStyle(data: AppData, nodeCount = 0) {
  ...
  return [
    ...
    { selector: 'node[type = "method"]', style: { shape: 'diamond' } },
    { selector: 'node[type = "lab_skill"], node[type = "skill"]', style: { shape: 'rectangle' } },
    { selector: 'node[type = "reaction_model"]', style: { shape: 'triangle' } },
    { selector: 'node[type = "common_error"]', style: { 'border-style': 'dashed', 'border-width': 2.5 } },
    {
      selector: 'edge',
      style: {
    ...
    {
      selector: 'edge[relation = "applies"]',
      style: { 'line-style': 'dashed', 'target-arrow-shape': 'triangle', 'curve-style': 'bezier' },
    },
    {
      selector: 'edge[relation = "derives"]',
      style: { 'line-style': 'dashed', 'target-arrow-shape': 'triangle', 'curve-style': 'bezier' },
    },
    {
      selector: 'edge[relation = "contrasts"]',
      style: { 'source-arrow-shape': 'triangle', 'target-arrow-shape': 'triangle', 'curve-style': 'bezier' },
    },
    {
      selector: 'edge[relation = "co_assessed"]',
      style: { 'line-style': 'dotted', 'target-arrow-shape': 'none', opacity: 0.48 },
    },
    {
