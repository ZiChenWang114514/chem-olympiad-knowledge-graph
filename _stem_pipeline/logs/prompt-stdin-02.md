...
3. 本 shard 任务文件：`_stem_pipeline/shards/shard-02/tasks.json`（XX 为你的编号）
...
- 进度：`_stem_pipeline/logs/shard-02-progress.jsonl`（每完成一题一行 JSON）
...
- 一张试卷多个 exam 对应同一 year-stage 时：按题号顺序匹配 problems 列表；题量不一致时优先匹配 number，剩余题写到 `_stem_pipeline/logs/shard-02-unmatched.md`
...
- 完成后在 `_stem_pipeline/logs/shard-02-DONE.txt` 写一行 `OK papers=N problems=M`
...
- shard tasks: _stem_pipeline/shards/shard-02/tasks.json
- stems out: _stem_pipeline/out/stems/{problemId}.json
- latex out: _stem_pipeline/out/latex/{problemId}.tex
- done marker: _stem_pipeline/logs/shard-02-DONE.txt
