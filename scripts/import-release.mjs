...
async function listFiles(dir) { try { const names=await readdir(dir,{withFileTypes:true}); const out=[]; for(const name of names){const p=join(dir,name.name); if(name.isDirectory()) out.push(...await listFiles(p)); else out.push(p)} return out } catch { return [] } }
...
export async function importRelease(releaseDir, outputDir) {
  ...
  const manifest=find('manifest.json')||{}; const releaseSequence=manifest.release_sequence??manifest.releaseSequence
  if(!Number.isInteger(releaseSequence) || releaseSequence<1) throw new Error('manifest.releaseSequence 必须是正整数')
  const taxonomy=find('taxonomy.json'); if(!taxonomy) throw new Error('缺少 taxonomy.json')
  ...
  const mappings=realExamPayloads.flatMap(x=>x.mappings||[]); const parts=realExamPayloads.flatMap(x=>x.parts||[]); const nodes=taxonomy.nodes||[]; const nodeById=new Map(nodes.map(n=>[n.id,n])); const problemIds=new Set(problems.map(problem=>problem.id)); const partById=new Map(parts.map(part=>[part.id,part]));
  for(const mapping of mappings){ if(!mapping.knowledge_node_id || !nodeById.has(mapping.knowledge_node_id)) throw new Error(`mapping 引用不存在的知识节点：${mapping.knowledge_node_id}`); if(mapping.target_kind==='problem' && !problemIds.has(mapping.target_id)) throw new Error(`mapping 引用不存在的题目：${mapping.target_id}`); if(mapping.target_kind!=='problem' && !partById.has(mapping.target_id)) throw new Error(`mapping 引用不存在的小问：${mapping.target_id}`) }
  for(const edge of taxonomy.edges||[]){ if(!nodeById.has(edge.source_node_id) || !nodeById.has(edge.target_node_id)) throw new Error(`edge 引用不存在的知识节点：${edge.id}`) }
  for(const problem of problems){
    const problemParts=parts.filter(part=>part.problem_id===problem.id).sort((a,b)=>(a.sort_order||0)-(b.sort_order||0))
    const partIds=new Set(problemParts.map(part=>part.id))
    const related=mappings.filter(m=>(m.target_kind==='problem'&&m.target_id===problem.id)||partIds.has(m.target_id))
    const nodeIds=[...new Set(related.map(m=>m.knowledge_node_id))]
    ...
    problem.partMappings=problemParts.map(part=>({partId:part.id,parentId:part.parent_id||undefined,label:part.label||'',nodeIds:[...new Set(mappings.filter(mapping=>mapping.target_id===part.id).map(mapping=>mapping.knowledge_node_id))],mappings:mappings.filter(mapping=>mapping.target_id===part.id).map(mapping=>({nodeId:mapping.knowledge_node_id,mappingRole:mapping.mapping_role,evidenceBasis:mapping.evidence_basis,evidencePages:JSON.parse(mapping.evidence_pages_json||'[]'),importance:mapping.importance}))}))
  }
  ...
  const outputs={'taxonomy.json':publicTaxonomy,'exams/index.json':{items:publicExams},'graph/all.json':graph,'search-index.json':{items:searchItems},'statistics.json':statistics}
  ...
  const stemFiles=await listFiles(join(out,'stems')); for(const file of stemFiles){const content=await readFile(file); const bytes=file.toLowerCase().endsWith('.json')?Buffer.byteLength(content.toString('utf8').replace(/\r\n/g,'\n')):content.byteLength; fileList.push({path:`data/${relative(out,file).replaceAll('\\','/')}`,bytes})}
  const publicManifest={dataVersion:manifest.data_version||manifest.dataVersion||manifest.version||'imported',schemaVersion:2,generatedAt:manifest.generated_at||manifest.generatedAt||new Date().toISOString(),releaseSequence,rightsPolicy:'metadata_and_stems_public',files:fileList,recordCounts:{exams:outputs['exams/index.json'].items.length,problems:problems.length,knowledgeNodes:graph.nodes.length,knowledgeEdges:graph.edges.length,problemStems:stemFiles.filter(file=>/problem-[0-9]{6}\.json$/i.test(file)).length,stemAssets:stemFiles.filter(file=>file.includes(`${file.includes('\\')?'\\':'/'}assets${file.includes('\\')?'\\':'/'}`)).length},notice:'本网站公开题目元数据、知识映射、题干和来源索引；答案与评分材料不随公开站点发布。'}
