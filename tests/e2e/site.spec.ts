...
test('节点文字列表选择后展示相邻、次序与题目', async ({ page }) => {
  ...
  await expect(page.getByTestId('node-pick-inorganic')).toHaveCount(0)
  await expect(page.getByTestId('node-pick-physical')).toHaveCount(0)
  await expect(page.getByTestId('node-pick-organic')).toHaveCount(0)
  await expect(page.getByTestId('node-pick-structural')).toHaveCount(0)
  await expect(page.getByTestId('node-pick-analytical')).toHaveCount(0)
  await expect(page.getByTestId('node-pick-experiment')).toHaveCount(0)
  await expect(picker.locator('[data-testid^="node-pick-"]')).toHaveCount(8)

  await page.getByTestId('node-pick-coordination').click()
  await expect(page.getByTestId('panel-title')).toHaveText('配位化学')
  await expect(page.getByTestId('neighbor-crystal')).toBeVisible()
  await expect(page.getByTestId('follow-crystal')).toBeVisible()
  await expect(page.getByTestId('related-problem-p-2024-final-01')).toBeVisible()
  await expect(page.getByTestId('open-knowledge')).toBeVisible()

  await page.getByTestId('follow-crystal').click()
  await expect(page.getByTestId('panel-title')).toHaveText('晶体场理论')
  await expect(page.getByTestId('prereq-coordination')).toBeVisible()
})
...
test('搜索定位知识节点并支持复位', async ({ page }) => {
  ...
  await page.getByTestId('suggest-knowledge-coordination').click()
  await expect(page).toHaveURL(/node=coordination/)
  await expect(page.getByTestId('panel-title')).toHaveText('配位化学')
